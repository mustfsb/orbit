...
- Keep `/todos` as the canonical task route and treat `/tasks` as redirect-only. (done)
- Reduce ambiguity between `/` and the leftover `/landing` route family. (done — `/landing` route removed)
